...
class SimuladorRoutesTestCase(unittest.TestCase):
    ...
    def test_enviar_dados_consumo_invalido(self):
        # Consumos com número incorreto de meses
        data_invalida_meses = {
            "consumos_kwh": [100, 120, 110, 130, 115],
            "tarifa_atual": 0.85
        }
        response = self.client.post('/simulador/enviar', json=data_invalida_meses)
        self.assertEqual(response.status_code, 400)
        response_json = json.loads(response.get_data(as_text=True))
        self.assertIn('erro', response_json)

        # Tarifa inválida
        data_invalida_tarifa = {
            "consumos_kwh": [100, 120, 110, 130, 115, 125],
            "tarifa_atual": 0
        }
        response = self.client.post('/simulador/enviar', json=data_invalida_tarifa)
        self.assertEqual(response.status_code, 400)
        response_json = json.loads(response.get_data(as_text=True))
        self.assertIn('erro', response_json)
    ...
    def test_obter_resultado_simulacao_sucesso(self):
        with self.app.app_context():
            ...
            simulacao = Simulacao(consumos_kwh=data['consumos_kwh'], tarifa_atual=data['tarifa_atual'], resultado={
        ...
        response_json = json.loads(response.get_data(as_text=True))
        self.assertIn('consumo_medio_mensal_kwh', response_json)
        self.assertEqual(response_json['num_placas_necessarias'], 3)
